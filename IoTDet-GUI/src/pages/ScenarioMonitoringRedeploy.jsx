...
import {deployMonitoring, getScenarioById} from "../services/ScenarioRequests.js";
import {
...
const ScenarioMonitoringRedeploy = () => {
                                ...
                                getScenarioById(scenarioId)
                                    .then(response4 => {
                                        ...
                                        for (let i = 0; i < response.data.length; i++) {
                                            const element2 = response.data[i];


                                            for (let j = 0; j < response4.data.devices.length; j++) {
                                                const element1 = response4.data.devices[j];

                                                if (element1.id_mender === element2.id_device) {
                                        ...
                                        let list_devices_selected = Promise.all(response4.data.devices.map((device) => {
                                            return getClassificationModelsByMonitoringScript(device.deployfile_monitoring_script_selected.monitoring_script.name)
                                                .then(response5 => {
                                                    return getAnomalyDetectionModelsByMonitoringScript(device.deployfile_monitoring_script_selected.monitoring_script.name)
                                                        .then(response6 => {
                                                            return {
                                                                mender_id: device.id_mender,
                                                                mac_address: device.mac_address,
                                                                device_type: device.device_type,
                                                                deployfileMonitoringScriptIdSelected: device.deployfile_monitoring_script_selected.id,
                                                                classificationModelsAvailable: response5.data,
                                                                anomalyDetectionModelsAvailable: response6.data,
                                                                classificationModelSelected: device.classification_model === null ? "" : device.classification_model.id,
                                                                anomalyDetectionModelSelected: device.anomaly_detection_model === null ? "" : device.anomaly_detection_model.id,
                                                                loadingModels: false,
                                                                isActivatedMitigation: device.is_activated_mitigation,
                                                                isActivatedConfigureTicks: device.is_activated_modify_ticks,
                                                                isActivatedClassificationAndAnomaly: device.is_activated_increment_classifier_anomaly,
                                        ...
                                        list_devices_selected.then(result => {

                                            for (let i = 0; i < result.length; i++) {
                                                if (result[i].classificationModelSelected !== "") {

                                                    for (let malware in response4.data.devices[i].tick_classification_classes) {

                                                        if (response4.data.devices[i].tick_classification_classes.hasOwnProperty(malware)) {

                                                            let listDeployfilesMitigationScriptSelectedByMalware = response4.data.devices[i].deployfiles_mitigation_script_selected.filter(el => el.malware_name === malware)
                                                            let mitigationMechanisms = []
                                                            for (let j = 0; j < listDeployfilesMitigationScriptSelectedByMalware.length; j++) {
                                                                mitigationMechanisms.push({
                                                                    deployfileMitigationScriptSelected: listDeployfilesMitigationScriptSelectedByMalware[j].deployfile_mitigation_script.id,
                                                                    json: listDeployfilesMitigationScriptSelectedByMalware[j].parameters !== null ? JSON.stringify(listDeployfilesMitigationScriptSelectedByMalware[j].parameters, null, 2) : ""
                                                                })

                                                            }

                                                            result[i].classificationMalware.push({
                                                                malware: malware,
                                                                cont: response4.data.devices[i].tick_classification_classes[malware],
                                                                mitigationMechanisms: mitigationMechanisms
                                                            })
...
export default ScenarioMonitoringRedeploy